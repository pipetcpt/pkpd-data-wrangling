...
![PK 분석 End-to-End 파이프라인 (End-to-End PK Analysis Pipeline). Import → QC/Clean → EDA → NCA → Report의 5단계 분석 파이프라인을 도식화하고, 각 단계에서 사용하는 R 패키지(readr, dplyr, ggplot2, NonCompart, Quarto)를 아이콘으로 표시하였다.](../images/ch09_fig1.png){#fig-ch09-1 width=100%}

@fig-ch09-1 은 PK 데이터 분석의 전체 워크플로우를 보여주며, 다음과 같이 5단계로 요약할 수 있습니다:
...
![PK 프로젝트의 Git 버전 관리 (Git Version Control for PK Projects). init → commit(data import) → commit(QC) → branch(sensitivity analysis) → merge → tag(final report)의 시간 축을 따른 Git 워크플로우를 타임라인 형태로 도식화하였다.](../images/ch09_fig3.png){#fig-ch09-3 width=100%}

### Git이란

**Git**은 코드와 문서의 변경 이력을 추적하는 **분산 버전 관리 시스템(Distributed Version Control System)**입니다 (@fig-ch09-3). PK 분석에서 Git을 사용해야 하는 이유:
...
![용량 비례성 평가 개념 (Dose Proportionality Assessment). Power model(AUC = α × Dose^β)에서 β=1(비례), β<1(미만비례), β>1(초과비례)의 세 가지 경우를 log-log 그래프 위에 표시하였다. 용량 비례성 판단 기준과 임상적 의미를 함께 설명한다.](../images/ch09_fig2.png){#fig-ch09-2 width=100%}

### 용량 비례성이란

**용량 비례성(Dose Proportionality)**이란 약물의 용량을 증가시킬 때 체내 노출(AUC, C~max~ 등)이 **비례적으로 증가**하는 것을 의미합니다 (@fig-ch09-2). 즉, 용량을 2배로 하면 AUC도 2배가 되어야 합니다.
