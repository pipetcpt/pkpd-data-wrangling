...
PK 데이터에서는 같은 데이터가 **Wide format**과 **Long format** 두 가지 형태로 존재할 수 있으며, 분석 목적에 따라 변환이 필요합니다 (@fig-ch04-1).

### Wide format vs Long format

![Wide ↔ Long 데이터 형태 변환 (Wide vs Long Data Format). PK 농도 데이터의 Wide 형태(열: T0, T1, T2...)와 Long 형태(행: ID, TIME, CONC)를 양방향 화살표로 연결하여 pivot_longer/pivot_wider 변환을 도식화하였다. 각 형태가 적합한 분석 상황을 함께 표시하였다.](../images/ch04_fig1.png){#fig-ch04-1 width=100%}
...
![BLQ 처리 방법 의사결정 트리 (BLQ Handling Methods Decision Tree). BLQ 검출 시 의사결정 과정을 트리 형태로 도식화하였다. Pre-dose 여부에 따라 Method M1(0으로 대체), Post-Cmax 여부에 따라 M5(LOQ/2) 또는 M3(censored 처리) 등의 방법을 선택하는 과정을 보여준다.](../images/ch04_fig3.png){#fig-ch04-3 width=100%}

@fig-ch04-3 은 BLQ 데이터의 처리 과정을 보여줍니다. BLQ 데이터의 처리는 약동학 분석에서 매우 중요한 통계적/약리학적 의미를 갖습니다. BLQ 값을 어떻게 처리하느냐에 따라 PK 파라미터 추정치가 달라질 수 있습니다.
...
![임상시험 데이터 흐름 (Clinical Trial Data Flow). CRF(Case Report Form)에서 수집된 원시 데이터가 SDTM 도메인(DM, PC, EX, VS)으로 표준화되고, ADaM 데이터셋(ADPC, ADPD)을 거쳐 최종 분석 데이터셋으로 변환되는 전체 흐름을 도식화하였다.](../images/ch04_fig2.png){#fig-ch04-2 width=100%}

:::{.callout-note}
## 임상시험 데이터의 표준화

임상시험에서 수집되는 데이터는 국제적으로 표준화된 형식을 따릅니다 (@fig-ch04-2). 이 표준을 이해하면 실제 임상 데이터를 받았을 때 더 효율적으로 분석할 수 있습니다.
:::
