...
![R 데이터 타입 체계와 PK 데이터 매핑 (R Data Types Hierarchy). R의 기본 데이터 구조인 vector에서 matrix/array, list, data.frame/tibble로 이어지는 계층 구조를 도식화하였다. 각 구조에 PK 데이터 변수(concentration=numeric, route=factor, ID=character)가 어떻게 매핑되는지를 함께 나타낸다.](../images/ch02_fig1.png){#fig-ch02-1 width=100%}

@fig-ch02-1 은 R의 데이터 타입 체계와 PK 데이터 변수의 매핑 관계를 보여줍니다. R에서 모든 데이터는 특정한 **타입(type)**을 가집니다. 약동학 데이터를 다룰 때 각 변수가 어떤 타입인지를 정확히 이해하는 것은 데이터 오류를 방지하고 올바른 분석을 수행하는 데 핵심적입니다.
...
![PK 데이터의 데이터프레임 구조 (PK Data Structure as Data Frame). 데이터프레임의 행(rows)은 개별 관측값, 열(columns)은 변수(ID, TIME, CONC, DOSE)에 대응하는 구조를 보여준다. Methotrexate 건선 환자의 실제 PK 데이터 예시를 통해 각 열의 데이터 타입과 의미를 설명한다.](../images/ch02_fig2.png){#fig-ch02-2 width=100%}

데이터프레임은 R에서 **표 형태의 데이터**를 저장하는 가장 핵심적인 구조입니다 (@fig-ch02-2). 약동학 데이터 분석의 대부분은 데이터프레임을 기반으로 이루어집니다. 데이터프레임은 사실 **동일한 길이의 벡터들을 열(column)으로 묶은 리스트**입니다.
...
![Methotrexate의 작용기전 (Methotrexate Mechanism of Action). Methotrexate가 DHFR(디히드로엽산 환원효소)을 억제하여 엽산 경로를 차단하고, 빠르게 분열하는 세포(건선의 각질세포)에 미치는 영향을 도식화하였다. 저용량에서의 아데노신 방출 증가를 통한 항염증 효과 경로도 함께 나타낸다.](../images/ch02_fig3.png){#fig-ch02-3 width=100%}

:::{.callout-note}
### Methotrexate (MTX) 약리학 개요

@fig-ch02-3 은 Methotrexate의 작용기전을 도식화한 것입니다.
