...
![재현 가능한 연구 워크플로우 (Reproducible Research Workflow). 원시 데이터(Raw data)에서 R 스크립트를 통한 분석, Quarto를 이용한 보고서/출판물 생성까지의 전 과정을 도식화하였다. Git을 통한 버전 관리와 AI 코딩 어시스턴트(Claude Code)의 역할을 함께 나타내어, 현대적인 재현 가능한 PK/PD 연구의 워크플로우를 보여준다.](../images/ch01_fig3.png){#fig-ch01-3 width=100%}

### 신약 개발 과정에서의 PK/PD 분석 역할

신약이 환자에게 도달하기까지는 평균 10-15년의 기간과 수조 원의 비용이 소요됩니다. 이 과정에서 PK/PD 분석은 거의 모든 단계에 관여합니다. @fig-ch01-3 은 원시 데이터에서 최종 출판물까지 이어지는 재현 가능한 연구 워크플로우를 보여줍니다.
...
![ADME 과정의 개요 (Overview of ADME Processes). 약물이 체내에 투여된 후 흡수(Absorption), 분포(Distribution), 대사(Metabolism), 배설(Excretion)의 4단계를 거치는 과정을 도식화하였다. 경구(PO) 및 피하(SC) 투여 경로에서의 흡수, 피부/조직으로의 분포, 간의 CYP 효소에 의한 대사, 신장/담즙을 통한 배설 경로와 주요 약동학적 파라미터(F, Vd, CL)의 관계를 나타낸다.](../images/ch01_fig1.png){#fig-ch01-1 width=100%}

약동학을 이해하기 위한 가장 기본적인 프레임워크가 바로 **ADME**입니다 (@fig-ch01-1). ADME는 흡수(Absorption), 분포(Distribution), 대사(Metabolism), 배설(Excretion)의 약자로, 약물이 체내에 들어와서 나가기까지의 전 과정을 설명합니다.
...
![피부과 질환 치료제의 발전 스펙트럼 (Dermatologic Disease Treatment Spectrum). 전통적 전신치료제(MTX, CsA)에서 생물학적 제제(anti-TNF, anti-IL), 소분자 표적 치료제(JAK 억제제, PDE4 억제제)로 이어지는 피부과 면역 질환 치료의 발전 과정을 시간순으로 도식화하였다. 각 세대별 대표 약물과 표적 분자를 함께 표시하였다.](../images/ch01_fig2.png){#fig-ch01-2 width=100%}
...
피부과 면역 질환의 치료는 다음과 같은 단계적 발전을 거쳐왔습니다 (@fig-ch01-2):
