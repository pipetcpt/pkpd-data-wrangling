...
![Baseline 보정 방법 비교 (Baseline Correction Methods). 4개 패널로 Raw data, 절대 변화량(Y-B), 백분율 변화((Y-B)/B×100), 비율(Y/B)을 동일한 데이터에 적용하여 변환 결과를 비교하였다. 각 방법의 장단점과 적용 상황을 함께 설명한다.](../images/ch11_fig2.png){#fig-ch11-2 width=100%}

@fig-ch11-2 는 동일한 데이터에 서로 다른 baseline 보정 방법을 적용한 결과를 비교한 것입니다. PD 데이터 분석에서 가장 먼저 결정해야 할 사항은 **baseline 보정 방법**입니다. 기저값(baseline)은 약물 투여 전 측정한 PD 값으로, 개인마다 다른 출발점을 가지므로 치료 효과를 공정하게 비교하려면 적절한 보정이 필수적입니다.
...
![Placebo 반응과 치료 효과 분리 (Placebo Response and Treatment Effect). 치료군과 위약군의 시간 경과에 따른 반응 곡선을 함께 표시하고, 실제 약물 효과 = 치료군 반응 - 위약군 반응의 관계를 시각적으로 나타내었다.](../images/ch11_fig3.png){#fig-ch11-3 width=100%}

@fig-ch11-3 에서 보듯이, 피부과 임상시험에서 **placebo 효과(placebo response)**는 상당히 크며, 이를 적절히 고려하지 않으면 약물의 진정한 효과를 과대평가할 수 있습니다.
...
![피부과 PD 평가 도구 비교 (Dermatology PD Scoring Systems). PASI(4개 체표 부위 × 중증도), EASI(4개 부위 × 징후), SCORAD(A+B/5+C), IGA(0-4 척도)의 평가 체계를 병렬 스코어링 다이어그램으로 비교하였다. 각 도구의 계산 방법과 적용 질환을 함께 표시하였다.](../images/ch11_fig1.png){#fig-ch11-1 width=100%}

@fig-ch11-1 은 피부과에서 널리 사용되는 PD 평가 도구(PASI, EASI, SCORAD, IGA)의 평가 체계를 비교한 것입니다.
