...
![Sequential vs Simultaneous 분석 비교 (Sequential vs Simultaneous PK-PD Analysis). Sequential 접근(PK 먼저 → 고정 → PD)과 Simultaneous 접근(PK+PD 동시)의 두 가지 분석 방법을 흐름도로 비교하고, 각 방법의 장단점을 함께 표시하였다.](../images/ch13_fig2.png){#fig-ch13-2 width=100%}

@fig-ch13-2 은 PK-PD 관계를 분석하는 두 가지 근본적 접근 방식을 비교합니다:
...
![노출-반응 분석 프레임워크 (Exposure-Response Analysis Framework). 세 개의 연결된 그래프로 Dose→Exposure(PK), Exposure→Efficacy(E-R), Exposure→Safety(E-S) 관계를 나타내고, 최적 용량 범위를 하이라이트하였다.](../images/ch13_fig1.png){#fig-ch13-1 width=100%}

### 용량-노출 관계 (Dose-Exposure Relationship)

@fig-ch13-1 에서 보듯이, 노출-반응 분석의 첫 단계는 **용량과 노출 지표의 관계**를 확인하는 것입니다. 이는 이전 장에서 학습한 용량 비례성(dose proportionality) 분석의 연장선입니다.
...
![용량 선택을 위한 유익성-위해성 평가 (Benefit-Risk Assessment for Dose Selection). 노출(exposure)에 따른 유효성(EASI-75 달성 확률) 곡선과 안전성(림프구감소증 위험) 곡선을 겹쳐 표시하고, 치료 창(therapeutic window)을 하이라이트하였다.](../images/ch13_fig3.png){#fig-ch13-3 width=100%}

@fig-ch13-3 은 유익성-위해성 평가의 개념적 프레임워크를 보여줍니다.
